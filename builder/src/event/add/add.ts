...
import { determineCommandKind, ensureCurrentlyInProjectRoot, ensureFolderExists, loadTemplate } from '@root/common';
    ...
    command
        ...
        .action((event, command, aggregate, options) => {
            ...
            fs.writeFileSync(
                evolveFilePath,
                ejs.render(
                    loadTemplate(__dirname, `./templates/evolve-${determineCommandKind(aggregate, command)}.ts.ejs`),
                    {
